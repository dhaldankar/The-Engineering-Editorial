---
name: wireloom
description: Specialist for authoring Wireloom DSL wireframe mockups inside Markdown. Use when the user asks to mock up, sketch, or wireframe a UI screen/dialog/flow, wants a picture of a layout in a doc/PR/issue instead of a prose description, or wants a mockup with callouts/annotations. Not for flowcharts or process diagrams (use Mermaid), interactive prototypes, or real working components — those need actual framework code instead.
